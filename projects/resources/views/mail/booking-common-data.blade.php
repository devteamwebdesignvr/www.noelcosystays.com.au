	...
	<tbody>
       ...
       @foreach($main_data['data']['components'] as $c)
            @if($c['isIncludedInTotalPrice']==0)
				...
				<tr>
					<td align="left" colspan="4" style="padding: 10px; font-weight: bold; font-size: 15px; color:#000; border: 1px solid #C81D1E; border-right:0px solid #C81D1E;" valign="top"><strong>{{$c['title']}} :</strong></td>
					<td align="center" style="padding: 10px; font-weight: bold; font-size: 15px; color:#000; border: 1px solid #C81D1E;" valign="top">{!! $setting_data['payment_currency'] !!}{{number_format($c['total'],2)}}</td>
				</tr>
           @endif
       @endforeach

   ...
   var checkin = '';
    var checkout = '';
     $(document).on("change","#property-selector",function(){
        ...
        url="{{ route('get-checkin-checkout-data-ajax') }}";
    ...
    function ajaxCallingData(){
        ...
        if($("#property-selector").val()!=""){
             if($("#txtFrom").val()!=""){
                 if($("#txtTo").val()!=""){
                    if($("#adult_data").val()>0){
                         $.post("{{route('get-checkin-checkout-data-ajax')}}",{start_date:$("#txtFrom").val(),end_date:$("#txtTo").val(),pet_fee_data_guarav:pet_fee_data_guarav,adults:adults,childs:childs,book_sub:true,property_id:$("#property-selector").val(),extra_discount:$("#extra-discount").val()},function(data){
                            if(data.status==400){
                                $("#submit-button-gaurav-data").hide();

                ...
                <tbody>
                    ...
                    @foreach($data as $client)
                        <tr>
                            ...
                            <td>{{ $client->price }}</td>
                            <td>
                                  @if($client->type_of_price=="default")
                                    {!! $setting_data['payment_currency'] !!} {{number_format($client->price,2)}}  
                                    @elseif($client->type_of_price=="weeklyby_day")
                                    
                                    @elseif($client->type_of_price=="monthly")
                                    {!! $setting_data['payment_currency'] !!} {{number_format($client->monthly_price,2)}} 
                                    @elseif($client->type_of_price=="weekly")
                                    {!! $setting_data['payment_currency'] !!} {{number_format($client->weekly_price,2)}}  
                                    @endif
                            </td>

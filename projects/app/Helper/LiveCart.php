...
class LiveCart{
    ...
    public function DifferentDates($start=false,$end=false,$format = 'Y-m-d'){
        $array = array();
   ...
   public function iCalDataCheckInCheckOut($property_id){
        $abc=HostAwayProperty::find($property_id);
        if($abc):
            $data=HostAwayBooking::where(['listingMapId'=>$abc->host_away_id])->whereIn("status",["new","modified","ownerStay"])->where("departureDate",">",date('Y-m-d'))->get();
            $checkin=[];
            $checkout=[];
            foreach($data as $row){
                $todate = date('Y-m-d',strtotime('-1 days',strtotime($row->departureDate)));
                $fdate = date('Y-m-d',strtotime('+1 days',strtotime($row->arrivalDate)));
                $checkin = array_merge($checkin,$this->DifferentDates($row->arrivalDate,$todate));
                $checkout = array_merge($checkout,$this->DifferentDates($fdate,$row->departureDate));
            }
            sort($checkin);
            sort($checkout);
    ...
    public function iCalDataCheckInCheckOutCheckinCheckout($property_id){
        ...
        if($abc):
            ...
            foreach($data as $row){
                ...
                $todate = date('Y-m-d',strtotime($row->departureDate));
                $fdate = date('Y-m-d',strtotime($row->arrivalDate));
                $now = strtotime($row->arrivalDate); 
                $your_date = strtotime($row->departureDate);
                $datediff =  $your_date-$now;
                $day= ceil($datediff / (60 * 60 * 24));
                for($i=1;$i<$day;$i++){
                    $date = strtotime($row->arrivalDate);
                    $date = strtotime("+".$i." day", $date);
                    $blocked[]= date('Y-m-d', $date);
                }
            }
        ...
        if($abc):
            //$blocked_date_from_calender=HostAwayDate::where(['hostaway_id'=>$abc->host_away_id])->whereIn("status",["blocked"])->get();
            /**$blocked_date_from_calender=HostAwayDate::where(['hostaway_id'=>$abc->host_away_id])->whereIn("status",["blocked","reserved"])->get();
            foreach($blocked_date_from_calender as $b){
                $blocked[]=$b->single_date;
            } **/
      
            $unavailable_dates = HostAwayDate::where("hostaway_id", $abc->host_away_id)
                  ->whereIn("status", ["blocked","reserved"])
                  ->orderBy("single_date")
                  ->pluck("single_date")  // Get only the dates
                  ->map(fn($d) => \Carbon\Carbon::parse($d)->toDateString())  // Convert to Y-m-d
                  ->unique()
                  ->values()
                  ->toArray();
              ...
              foreach ($unavailable_dates as $i => $date) {
                  $current = \Carbon\Carbon::parse($date);
                  $prev = $current->copy()->subDay()->toDateString();
                  $next = $current->copy()->addDay()->toDateString();

                  $is_prev_blocked = in_array($prev, $unavailable_dates);
                  $is_next_blocked = in_array($next, $unavailable_dates);

                  if (!$is_prev_blocked && $is_next_blocked) {
                      $checkin[] = $date;
                  } elseif ($is_prev_blocked && !$is_next_blocked) {
                      $checkout[] = $date;
                  } elseif (!$is_prev_blocked && !$is_next_blocked) {
                      // single-day block
                      $checkin[] = $date;
        ...
        sort($checkin); sort($checkout); sort($blocked);
        return ["checkin"=>$checkin,"checkout"=>$checkout,"blocked"=>$blocked];
    }

     public function refreshIcalData($property_id,$ical_link,$id){
    ...
    public function allIcalImportListRefresh(){
